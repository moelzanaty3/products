...
The working directory for this project is the same folder the README you are reading right now is located. All commands assume you are in the `{PROJECT_ROOT}/apps/mentomz` in your console.
...
From `apps/mentomz` you can run the following command to validate your environment:
...
```shell
DATABASE_URL="mysql://root@localhost:3309/mentomz"
```
...
The first time you run this command it will seed the database with the contents of `apps/mentomz/seed_data`
...
```bash
pscale connect mentomz BRANCH_NAME --port 3309
```
...
The `dev:stripe` node script is a shorthand for `stripe listen --forward-to localhost:3000/api/skill/webhook/stripe`.
...
```bash
pscale connect mentomz PLANETSCALE_DB_BRANCH_NAME --port 3309
```
...
```bash
pscale branch create mentomz PLANETSCALE_DB_BRANCH_NAME
```
...
```bash
pscale deploy-request create mentomz descriptive-branch-name
```
...
```bash
pscale database restore-dump mentomz next-steps --dir ./seed_data
```
...
```bash
pscale database dump mentomz main --output ./seed_data/pscale_data_dump
```

Now the `seed_data/pscale_data_dump` folder holds a database dump you can restore to your branch:

```bash
pscale database restore-dump mentomz next-steps --dir ./seed_data/pscale_data_dump --overwrite-tables
```

## Edit content

Edit workshops with Sanity at [mentomz.sanity.studio](https://mentomz.sanity.studio/).
